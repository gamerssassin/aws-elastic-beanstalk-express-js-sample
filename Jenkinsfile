        ...
        stage('Security: Snyk (deps)') {
            steps {
                withCredentials([string(credentialsId: 'snyk-token', variable: 'SNYK_TOKEN')]) {
                    ...
                    ' [-f.env] && .. / .env || true
                    # Export token
                    for CLI(do NOT echo it)
                    export SNYK_TOKEN = "$(printf '%s' "
                    $SNYK_TOKEN " | tr -d '\\r\\n')"
                    # Optional: set region endpoint(non - secret).Put SNYK_API in .env
                    if you’ re on EU / AU.
                        [-n "${SNYK_API:-}"] &&
                        export SNYK_API
                    set - o pipefail
        ...
        stage('Security: Snyk (container)') {
            steps {
                withCredentials([string(credentialsId: 'snyk-token', variable: 'SNYK_TOKEN')]) {
                    ...
                    $SNYK_TOKEN " | tr -d '\\r\\n')"
                    set - o pipefail
                    snyk container test "$DOCKERHUB_USER/$IMAGE:$BUILD_NUMBER"\
                        --file = Dockerfile\
                        --severity - threshold = high\
                        --exclude - base - image - vulns | tee snyk - image.txt ''
                    '
                }
            }
            post {
                failure {
                    echo 'Security scan failed due to High/Critical vulnerabilities (non-base layers).'
                }
            }
        }
        ...
        stage('Push image') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub', usernameVariable: 'DH_USER', passwordVariable: 'DH_PASS')]) {
                    ...
                    printf '%s'
                    "$DH_PASS" | docker login - u "$DOCKERHUB_USER"--password - stdin
                    docker push "$TAG"
                    docker tag "$TAG"
                    "$DOCKERHUB_USER/$IMAGE:latest"
